<form action="" method="post" name="f_conf">
    ...
    <table class="table table-hover table-borderless ctable">
        <tbody>
            <tr>
                <td class="table-primary col-sm-2">請求番号</td>
                <td>
    ...
    <table class="table table-hover table-borderless ctable">
        <tbody>
            ...
            <tr>
                <td class="table-primary col-sm-2">実施日</td>
                <td>
                    <div class="form-control rounded-0 ">
                        <?php echo $_POST['CompletionDate'];
                        if (!empty($_POST['CompletionDate2'])) {
                            echo "　～　" . $_POST['CompletionDate2'];
                        }
                        ?>
                        <input type="hidden" name="CompletionDate" value="<?= $_POST['CompletionDate'] ?>">
                        <input type="hidden" name="CompletionDate2" value="<?= $_POST['CompletionDate2'] ?>">
                    </div>
                </td>
            </tr>
        </tbody>
    ...
    <table class="table table-hover table-borderless ctable">
        <tbody>
            <tr>
                <td class="table-primary col-sm-2">支払期日</td>
                <td>
                    <div class="form-control rounded-0 ">
                        <?= $_POST['PaymentDueDate'] ?>
                        <input type="hidden" name="PaymentDueDate" value="<?= $_POST['PaymentDueDate'] ?>">
                    </div>
                </td>
            </tr>
            <tr>
                <td class="table-primary col-sm-2">記載振込先</td>
                <td>
                    <?php
                    $banksql = "SELECT BankID,concat_ws(Char(9),BankName,BankBranch,AccountType,AccountNumber)as banks FROM bank WHERE BankID={$_POST['BankID1']} ORDER BY BankID ASC;";
                    $stbank1 = $dbh->query($banksql);
                    $bank1 = $stbank1->fetch();
                    ?>
                    <div class="form-control rounded-0 mb-1">
                        １.<?= $bank1['banks'] ?>
                        <input type="hidden" name="BankID1" value="<?= $bank1['BankID'] ?>">
                    </div>

                    <?php
                    $banksql = "SELECT BankID,concat_ws(Char(9),BankName,BankBranch,AccountType,AccountNumber)as banks FROM bank WHERE BankID={$_POST['BankID2']} ORDER BY BankID ASC;";
                    $stbank2 = $dbh->query($banksql);
                    $bank2 = $stbank2->fetch();
                    ?>
                    <div class="form-control rounded-0 mb-1">
                        ２.<?= $bank2['banks'] ?>
                        <input type="hidden" name="BankID2" value="<?= $bank2['BankID'] ?>">
                    </div>

                    <?php
                    $banksql = "SELECT BankID,concat_ws(Char(9),BankName,BankBranch,AccountType,AccountNumber)as banks FROM bank WHERE BankID={$_POST['BankID3']} ORDER BY BankID ASC;";
                    $stbank3 = $dbh->query($banksql);
                    $bank3 = $stbank3->fetch();
                    ?>
                    <div class="form-control rounded-0 ">
                        ３.<?= $bank3['banks'] ?>
                        <input type="hidden" name="BankID3" value="<?= $bank3['BankID'] ?>">
                    </div>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="table-primary col-sm-2">入金日</td>
                <td>
                    <div class="form-control rounded-0 ">
                        <?= $_POST['PaymentDate'] ?>
    ...
    <table class="table table-hover table-bordered table-sm" style="width: 100%;">
        ...
        <tfoot>
            <tr>
                ...
                <td><input class="lists form-control rounded-0 px-1 text-right" type="text" value="<?= number_format($billsum, 3) ?>"></td>
            </tr>
            <tr>
                <td class=" table-primary">消費税</td>
                <?php
                if ($_POST['Tax'] === "1.08" or $_POST['Tax'] === "1.1") {
                    $tax = bcsub($billsum, bcdiv($billsum, $_POST['Tax'], 3), 3);
                    $tax2 = 0;
                } elseif ($_POST['Tax'] === "0.1" or $_POST['Tax'] === "0.08") {
                    $tax = bcmul($billsum, $_POST['Tax'], 3);
                    $tax2 = $tax;
                }
                ?>
            ...
            <tr>
                ...
                <td><input class="lists form-control rounded-0 px-1 text-right" type="text" value="<?= number_format($billsum + $tax2, 3) ?>">
                    <input type="hidden" name="billsum" value="<?= $billsum ?>">
                </td>
            </tr>
        </tfoot>
    </table>
    ...
    <div style="width:100%;text-align:center;" class="pb-5">
        ...
        <input class="btn btn-secondary rounded-0 btn-sm px-4" type="submit" formmethod="post" formaction="bill/preview" formtarget="_blank" value="請求書">
        <input class="btn btn-secondary rounded-0 btn-sm px-4" type="submit" formmethod="post" formaction="bill/deliveryslip" formtarget="_blank" value="納品書">
