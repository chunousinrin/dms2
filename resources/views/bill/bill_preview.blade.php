...
<head>
    ...
    <style>
        ...
        * {
        ...
        .billlist tr td:nth-child(n+2) p {
            display: none;
        }
...
<body style="position: relative;">
    ...
    <div class="wrap">
        <header class="billhead">
            <ul>
                <li>
                    ...
                    <?php
                    if ($_POST['classicationId'] == 15) {
                        echo "<div class='otln' style='margin-bottom:5mm;border:none'><div>　</div></div>";
                    } else {
                        echo "<div class='otln' style='margin-bottom:5mm;'>"
                            . "<span>実施日</span>"
                            . "<div>";
                        if (!empty($_POST['CompletionDate'])) {
                            echo wareki(substr($_POST['CompletionDate'], 0, 4)) . substr($_POST['CompletionDate'], 5, 2) . '月' . substr($_POST['CompletionDate'], 8, 2) . '日';
                        }
                        if (!empty($_POST['CompletionDate2'])) {
                            echo " ～ " . wareki(substr($_POST['CompletionDate2'], 0, 4)) . substr($_POST['CompletionDate2'], 5, 2) . '月' . substr($_POST['CompletionDate2'], 8, 2) . '日';
                        } else {
                            echo "&nbsp;";
                        }
                        echo "</div>"
                    ...
                    <div style="font-size: 13pt;font-weight:800;border-bottom:2px solid black;padding-left: 2mm;">
                        ...
                        if ($_POST['Tax'] == 0.08 | $_POST['Tax'] == 0.1) {
                            $sum = floor($_POST['billsum'] * (1 + $_POST['Tax']));
                        } elseif ($_POST['Tax'] == 1.08 | $_POST['Tax'] == 1.1) {
                            $sum = floor($_POST['billsum']);
                        }
                        echo "￥" . number_format($sum) . " -"; ?>
                    </div>
                    <div style="padding-left: 2mm;">
                        <?php if (empty($_POST['PaymentDueDate'])) {
                            echo "お支払期日：ご請求日より45日以内";
                        } else {
                            echo "お支払期日：" . wareki(substr($_POST['PaymentDueDate'], 0, 4)) . substr($_POST['PaymentDueDate'], 5, 2) . '月' . substr($_POST['PaymentDueDate'], 8, 2) . '日';
                        }
                        ?>
                    </div>
                </li>
                    ...
                    <div class="otln" style="margin-bottom: 5mm;"><span>発行日</span>
                        <div>
                            ...
                            if ($_POST['cddisplay'] == 1) {
                                echo wareki(substr($_POST['CreatedDate'], 0, 4)) . substr($_POST['CreatedDate'], 5, 2) . '月' . substr($_POST['CreatedDate'], 8, 2) . '日';
                            } else {
                                echo "　　年　　月　　日";
                            } ?>
                        </div>
        ...
        <footer class="billfoot">
            ...
            <div>
                <?php
                $bank1sql = "SELECT * FROM bank WHERE BankID={$_POST['BankID1']}";
                $stbank1 = $dbh->query($bank1sql);
                $bank1 = $stbank1->fetch();
                $bank2sql = "SELECT * FROM bank WHERE BankID={$_POST['BankID2']}";
                $stbank2 = $dbh->query($bank2sql);
                $bank2 = $stbank2->fetch();
                $bank3sql = "SELECT * FROM bank WHERE BankID={$_POST['BankID3']}";
